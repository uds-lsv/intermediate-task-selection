...
class DataTrainingArguments:
    ...
    do_taskemb: Optional[bool] = field(
        default=False, metadata={"help": "If set, compute the task embeddings (FIM)."}
    )
...
def main():
    ...
    args_parser.add_argument("--do_taskemb", action='store_true')
    ...
    # Get task embedding arguments
    emb_args = TaskEmbArguments()
    ...
    # Run embeddings
    print("data_args.do_taskemb", data_args.do_taskemb)
    if data_args.do_taskemb:
        logger.info("do_taskemb")
        embeddings = trainer.run_embeddings(emb_args=emb_args)
