...
class Squad(AbstractTask):
    ...
    # For SQuAD v1.1,  We don't duplicates the samples based on
    # number of answers as we do for SuperGlue-MultiRC,
    # Because multiple answers only appear in development set.
...
class CR(AbstractTask):
    ...
    labelId2text = {"0": "negative", 
                    "1": "postive"}

    # "json", data_files="my_file.json"
    def load_dataset(self, split):
        d = datasets.load_dataset('json', data_files=f"/nethome/pjlin/pythonProjects/compacter/seq2seq/{split}.jsonl")
        return d["train"]
        
    def preprocessor(self, example, add_prefix=True):
        src_texts = ["text:", example['text']]
        # tgt_texts = [str(example['label'])]
        tgt_texts = [self.labelId2text[str(example['label'])]]
...
class STSB(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix=True):
        ...
        round_digit_in_string     = str(round_stsb_target(example['label']))
        round_digit_in_string_lst = [ self.labelId2text[ele] for ele in round_digit_in_string ]
        tgt_texts = [ " ".join(round_digit_in_string_lst) ]
        return self.seq2seq_format(src_texts, tgt_texts, add_prefix)
...
class CXC(AbstractTask):
    ...
    def load_dataset(self, split):
        d = datasets.load_dataset('json', data_files=f"/nethome/pjlin/pythonProjects/compacter/seq2seq/cxc.{split}")
        return d["train"]

    def preprocessor(self, example, add_prefix=True):
        src_texts = ["sentence1:", example['sentence1'],
                     "sentence2:", example["sentence2"]]
        tgt_texts = [str(round_stsb_target(float(example['agg_score'])))]
        round_digit_in_string     = str(round_stsb_target(float(example['agg_score'])))
        round_digit_in_string_lst = [ self.labelId2text[ele] for ele in round_digit_in_string ]
        tgt_texts = [ " ".join(round_digit_in_string_lst) ]
        return self.seq2seq_format(src_texts, tgt_texts, add_prefix)
...
class DROP(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix):
        ...
        # provide all answers for evaluation
        answers = list(set(example['answers_spans']['spans']))
        extra_fields = {"answers": answers}
        return self.seq2seq_format(source, target, add_prefix,
                                   extra_fields=extra_fields)
...
class HellaSwag(AbstractTask):
    name="hellaswag"
    # insturction_prefix = "Question: Which option best completes the sentence?"
    labels_list = ['W', 'X', 'Y', 'Z']
    metric = [metrics.accuracy]
    metric_names = ["accuracy"]
    split_to_data_split = {"train": "train",
    ...
    def load_dataset(self, split):
        """ Downloading and loading the BoolQ dataset from the hub. 
        """
        return datasets.load_dataset('hellaswag', split=split)
    
    def preprocessor(self, example, add_prefix=True):
        """
        Define the insturction here. `input_text` will be combined a one string
        with a unified key in rawdataset.
        """
        option = " W / X / Y / Z."
        option_text = " (W): \"{}\" / (X): \"{}\" / (Y): \"{}\" / (Z): \"{}\".".format(example["endings"][0],
                                                                               example["endings"][1],
                                                                               example["endings"][2],
                                                                               example["endings"][3])
        sentence = example["ctx"] + option
        # sentence = example["ctx"]
        src_texts = ["sentence: ", sentence,
                     f"{option_text}"]
        tgt_texts = [self.labelId2text[example["label"]]]
        return self.seq2seq_format(src_texts, tgt_texts, add_prefix)


#For acc (good performance one)
class Winogrande(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix=True):
        ...
        with a unified key in rawdataset.
        """
        option_text = "{} / {}".format(example["option1"],
                                       example["option2"])
        

        sentence = example["sentence"].replace("_", option_text) # replace
        # print("sentence:", sentence)
        # print("option_text:", option_text)
        #sentence = example["sentence"]

        options = "option1 {} option2: {}".format(example["option1"], example["option2"])
        src_texts = [f"entence: {sentence}",
                     options
                     ]
                     
        f" {option_text}. The correct answer is",

        lab = example["answer"].strip()
        if lab == "1":
            answer = example["option1"].strip()
            tgt_texts = [answer]
        
        elif lab == "2":
            answer = example["option2"].strip()
            tgt_texts = [answer]
        
        tgt_texts = [ self.labelId2text[example["answer"]]]
        return self.seq2seq_format(src_texts, tgt_texts, add_prefix)


class CosmosQA(AbstractTask):
    name = "cosmos_qa"
    split_to_data_split = {"train": "train",
                           "validation": "validation",
                           "test": "validation"}
    ...
    def preprocessor(self, example, add_prefix):
        answer = pad_punctuation(str(example['label'])) # int32 to str
        question = pad_punctuation(example['question'])
        context = pad_punctuation(example['context'])
        options = pad_punctuation("(0): {} / (1): {} / (2): {} / (3): {}".format(
            example["answer0"],
            example["answer1"],
            example["answer2"],
            example["answer3"])
        ...
        if answer == "0": 
            answer_text = example["answer0"]
        elif answer == "1":
            answer_text = example["answer1"]
        elif answer == "2":
            answer_text = example["answer2"]
        elif answer == "3":
            answer_text = example["answer3"]
        else:
            raise ValueError
        target = [answer_text]
        #target  = [ self.labelId2text[str(example['label'])]]
        return self.seq2seq_format(source, target, add_prefix)
...
class RACE(AbstractTask):
    ...
    def load_dataset(self, split):
        return datasets.load_dataset(self.name, "middle", split=split)

    def preprocessor(self, example, add_prefix):
        answer = pad_punctuation(example['answer'])
        question = pad_punctuation(example['question'])
        context = pad_punctuation(example['article'])
        options = pad_punctuation("(A): / {} / (B): {} / (C): {} / (D): {}".format(example["options"][0],
                                                                           example["options"][1],
                                                                           example["options"][2],
                                                                           example["options"][3]))
        ...
        if answer == "A": 
            answer_text = example["options"][0]
        elif answer == "B":
            answer_text = example["options"][1]
        elif answer == "C":
            answer_text = example["options"][2]
        elif answer == "D":
            answer_text = example["options"][3]
        else:
            raise ValueError
        # target = [answer_text]
        target = [self.labelId2text[str(answer).upper()]]
        return self.seq2seq_format(source, target, add_prefix)
