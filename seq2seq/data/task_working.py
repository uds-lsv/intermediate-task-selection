...
class Drop(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix):
        answer = pad_punctuation(example['answers_spans']['spans'])
        question = pad_punctuation(example['question'])
...
### max sequence 128 not enough
class HellaSwag(AbstractTask):
    name="hellaswag"
    insturction_prefix = "Given the context, answer which option finish the context."
    labels_list = ['0', '1', '2', '3']
    metric = [metrics.accuracy]
    metric_names = ["accuracy"]
    split_to_data_split = {"train": "train",
    ...
    def preprocessor(self, example, add_prefix=True):
        ...
        """
        option_text = "(0): {} (1): {} (2): {} (3): {}".format(example["endings"][0],
                                                       example["endings"][1],
                                                       example["endings"][2],
                                                       example["endings"][3],
                                                       )
        src_texts = [self.insturction_prefix,
                     " Context: ", example["ctx"],
...
class Winogrande(AbstractTask):
    name="winogrande"
    insturction_prefix = "Answer which option should be filled into the sentence."
    labels_list = ['1', '2']
    metric = [metrics.accuracy]
    metric_names = ["accuracy"]
    split_to_data_split = {"train": "train",
    ...
    def preprocessor(self, example, add_prefix=True):
        ...
        with a unified key in rawdataset.
        """
        option_text = "(1): {} (2): {}".format(example["option1"],
                                               example["option2"]
                                               )
        src_texts = [self.insturction_prefix,      
                    "Sentence: " + example["sentence"],
...
class CosmosQA(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix):
        answer = pad_punctuation(str(example['lable'])) # int32 to str
        question = pad_punctuation(example['question'])
        context = pad_punctuation(example['context'])
        options = pad_punctuation("(0) {} (1) {} (2) {} (3): {}".format(example["answer0"],
                                                                        example["answer1"],
                                                                        example["answer2"],
                                                                        example["answer3"]))
        source = ["question:", question,
...
class Race(AbstractTask):
    ...
    def preprocessor(self, example, add_prefix):
        answer = pad_punctuation(example['answers'])
        question = pad_punctuation(example['question'])
...
# d = datasets.load_dataset("HuggingFaceM4/COCO", "2014_captions", split="train")
